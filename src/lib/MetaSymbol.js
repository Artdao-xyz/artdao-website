...
export default class MetaSymbol
{
    ...
    changeTexture(_index)
    {
        ...
        {
            this.material.matcap = this.textures[_index]
            this.textureMixD =0.0
        }
        this.backMixD = 1.0
        this.backMix = 0.0
    ...
    update()
    {
        ...
        this.fixedTable.map((fixed,index)=>{
            ...
            ///console.log(index,Math.abs(this.scrollPercentD-(fixed*100.0)))
        ...
        this.scrollPercent+=(this.scrollPercentD-this.scrollPercent)*.05
        this.textureMix+=(this.textureMixD-this.textureMix)*.05
        ...
        for(var i=0;i<this.particleCount;i++)
        {
            ...
            this.metaball.addBall(.5-scale*.5+pos.x*scale,.5+scale*.5-pos.y*scale,.5,pos.z*sScale,32)
        }
        this.metaball.update()

        this.metaball.position.y = Math.sin(time*config.default.APP.LEVITATION_VEL)*config.default.APP.LEVITATION_AMP;
        this.metaball.rotation.y = THREE.MathUtils.degToRad(Math.cos(time*config.default.APP.ROT_Y_VEL)*config.default.APP.ROT_Y_AMP);
